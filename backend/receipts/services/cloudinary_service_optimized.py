...
class CloudinaryReceiptService:
    ...
    MIN_BYTES = int(os.getenv('CLOUDINARY_MIN_BYTES', '40960'))  # 40KB minimum
    ...
    def upload_receipt_image(
        ...
        try:
            ...
            if len(optimized_data) < self.MIN_BYTES:
                logger.info(f"Image too small ({len(optimized_data)} bytes < {self.MIN_BYTES}), skipping Cloudinary upload")
                return {
                    'success': False,
