...
const DashboardCard = ({ title, icon, value, color }) => {
  return (
    <div style={{ 
      ...
      <div style={{ fontSize: '24px', marginBottom: '8px', color: color }}>{icon}</div>
      <h3 style={{ margin: '0 0 8px 0', fontSize: '18px', color: '#333' }}>{title}</h3>
      <p style={{ margin: 0, fontSize: '22px', fontWeight: 'bold', color: color }}>{value}</p>
    </div>
  );
};
